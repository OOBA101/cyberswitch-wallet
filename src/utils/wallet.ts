...
const getOrCreateDeviceKey = async (): Promise<CryptoKey> => {
  ...
  await store.set(ENC_KEY_KEY, buf2hex(exported as ArrayBuffer))
  return key
}
...
export const getEncryptionKey = async (): Promise<CryptoKey> => {
  ...
  if (hasPwd) {
    // Password is set but _cachedKey is null — wallet is locked.
    // Callers must not call this before verifyPassword() succeeds.
    throw new Error('WALLET_LOCKED')
  }
  // No password — use device key
...
const hashPassword = async (password: string): Promise<string> => {
  ...
  return buf2hex(bits as ArrayBuffer)
}

export const setPassword = async (password: string): Promise<void> => {
  // Load wallets with CURRENT key BEFORE switching — prevents DOMException
...
export const removePassword = async (): Promise<void> => {
  if (!_cachedKey) {
    console.error('removePassword: no cached key — wallet may be locked')
    return
  }
  // Load with CURRENT (password) key BEFORE switching
  const currentWallets = await loadWallets()
...
export const loadWallets = async (): Promise<WalletData[]> => {
  try {
    ...
    // Migrate old unencrypted format
    ...
    console.error('loadWallets error:', e?.name, e?.message)
    return []
  }
}
